##########################################################################
##########################################################################
#Ranked probability skill score for 3d fields (for tercile categories)
...
#          rpsstwo3d(data1,data2)

rpsstwo3d <- function(data1,data2) {
...
for (i in 1:nlon){
      for(j in 1:nlat){
              ...
              bsclim1[i,j]<- mean(((1/3)-binobs1[i,j,])^2,na.rm = TRUE)
              bsclim2[i,j]<- mean(((2/3)-binobs2[i,j,])^2,na.rm = TRUE)
      }
}
.Fortran("rkeepalive",i=as.integer(6),n=as.integer(6))

rps<-bs1+bs2
rpsclim<-bsclim1+bsclim2
