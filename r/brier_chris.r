...
# Author:
#
#      Chris Ferro <c.a.t.ferro@reading.ac.uk> 12 April 2006
#
# References:
#
#      Ferro CAT (2006) Comparing probabilistic forecasting systems with
#      the Brier score. Submitted. Available from
#
...
brier.score <- function(x, y = NULL, th, size, above = TRUE, con = TRUE, unc = TRUE, cover = NULL, boot = 1000, simul = FALSE) {
  ...
  xs.con <- temp$s.con
  xs.unc <- temp$s.unc
  xout <- list(members = size, thresholds = th, scores = xb, reliability = temp$rel, resolution = temp$res, uncertainty = temp$uncer)
  ...
  if(!is.null(y)) {
    ...
    ys.con <- temp$s.con
    ys.unc <- temp$s.unc
    yout <- list(members = size, thresholds = th, scores = yb, reliability = temp$rel, resolution = temp$res, uncertainty = temp$uncer)
...
brier.point <- function(obs, sim, th, size, con, unc) {
  ...
  for(i in 1:nt) {
    ...
    for(j in 1:ns) {
      b[i, j] <- brier(I, Q, m, size[j])
      b0[i, j] <- brier.true(I, Q, size[j])
      if(con) s.con[i, j] <- brier.sd(I, Q, m, n, size[j])
      if(unc) s.unc[i, j] <- brier.sd(I, Q, m, n, size[j], TRUE)
    }
    Ibar <- mean(I)
    for(k in 1:n) Qbar[k] <- mean(I[Q == Q[k]])
    rel[i] <- mean((Q - Qbar)^2)
    res[i] <- mean((Qbar - Ibar)^2)
    uncer[i] <- Ibar * (1 - Ibar)
  }
  list(b = b, b0 = b0, s.con = s.con, s.unc = s.unc, rel = rel, res = res,
  uncer = uncer)
}
...
brier.cov <- function(xobs, xsim, yobs, ysim, th, size) {
  ...
  for(i in 1:nt) {
    ...
    for(j in 1:ns) {
      M <- size[j]
      Wx <- (1 - 1 / M) * Qx * (m * Qx - (1 - m / M) / (1 - 1 / M)) / (m - 1) + (1 - 2 * Qx) * Ix
      Wy <- (1 - 1 / M) * Qy * (m * Qy - (1 - m / M) / (1 - 1 / M)) / (m - 1) + (1 - 2 * Qy) * Iy
...
brier <- function(I, Q, m, M) {
  mean((1 - 1 / M) * Q * (m * Q - (1 - m / M) / (1 - 1 / M)) / (m - 1) + (1 - 2 * Q) * I)
}


brier.sd <- function(I, Q, m, n, M, sample = FALSE) {
  if(sample) {
    return(sd((1 - 1 / M) * Q * (m * Q - (1 - m / M) / (1 - 1 / M)) / (m - 1) + (1 - 2 * Q) * I) / sqrt(n))
  }
  if(m < 4) {
    Q2 <- Q * Q
    Q3 <- Q * Q2
  ...
  if(con) {
    ...
    for(k in 1:boot) {
      ...
      for(i in 1:nt) {
        ...
        for(j in 1:ns) {
          xbb[i, j, k] <- brier(I, Q, m, size[j])
          xsb[i, j, k] <- brier.sd(I, Q, m, n, size[j])
        }
      }
    }
    ...
    if(!is.null(yobs)) {
      for(k in 1:boot) {
        ...
        for(i in 1:nt) {
          ...
          for(j in 1:ns) {
            ybb[i, j, k] <- brier(I, Q, m, size[j])
            ysb[i, j, k] <- brier.sd(I, Q, m, n, size[j])
          }
        }
      }
    ...
    if(is.null(yobs)) {
      for(k in 1:boot) {
        ...
        for(i in 1:nt) {
          ...
          for(j in 1:ns) {
            xbb[i, j, k] <- brier(I, Q, m, size[j])
            xsb[i, j, k] <- brier.sd(I, Q, m, n, size[j], TRUE)
          }
        }
      }
      out <- list(x = list(score = xbb, std.err = xsb))
      ...
      for(k in 1:boot) {
        ...
        for(i in 1:nt) {
          ...
          for(j in 1:ns) {
            xbb[i, j, k] <- brier(Ix, Qx, m, size[j])
            xsb[i, j, k] <- brier.sd(Ix, Qx, m, n, size[j], TRUE)
            ybb[i, j, k] <- brier(Iy, Qy, m, size[j])
            ysb[i, j, k] <- brier.sd(Iy, Qy, m, n, size[j], TRUE)
          }
        }
